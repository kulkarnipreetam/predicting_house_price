...
train_R2 = xgb_regressor.score(X_train,y_train)

val_R2 = xgb_regressor.score(X_val,y_val)
...
for threshold in threshold_values:
    ...
    xgb_regressor.fit(X_modified_train, y_modified_train)
    
    Feature_selection_df.loc[Feature_selection_df["Threshold"]==threshold,"R2"] = xgb_regressor.score(X_modified_val,y_modified_val)
...
'''
Looking at the threshold vs. validation R2 graph, we can see that 0.002 is appropriate as it performs 
...
param_grid = {'n_estimators': randint(300, 1000),
...
optimized_xgb_model = xgb.XGBRegressor(**best_params, early_stopping_rounds=7)

optimized_xgb_model.fit(X_modified_train, y_modified_train,
                        eval_set=[(X_modified_val, y_modified_val)],
                        verbose = False)

X_test_selected = X_test[X_modified.columns]

Training_R2 = optimized_xgb_model.score(X_modified_train, y_modified_train)
Final_R2 = optimized_xgb_model.score(X_test_selected, y_test)
...
log_preds = optimized_xgb_model.predict(onehot_encoded_test_df)
