...
for threshold in threshold_values:
    ...
    rf_regressor.fit(X_modified_train, y_train)
    
    Feature_selection_df.loc[Feature_selection_df["Threshold"]==threshold,"R2"] = rf_regressor.score(X_modified_test,y_test)
...
'''
Looking at the threshold vs. R2 graph, 0.01, 0.02, 0.03 and 0.04 can be a good choice of threshold as the R2 is stable upto 0.04 
...
retaining more features i.e., by using thresholds of 0.02 or 0.03.
'''
thresholds_for_further_evaluation = [0.01, 0.02, 0.03, 0.04]
hyperparameter_dict = {}
...
for i in range(len(thresholds_for_further_evaluation)):
    ...
    optimized_rf_model.fit(X_modified_train, y_train)
    
    R2 = optimized_rf_model.score(X_modified_test,y_test)
    
    print("\nR2 of model after hyperparameter tuning: {}".format(R2))
    
'''
After evaluating thresholds while varying hyperparameters, we can see that 0.02 is appropriate as it performs 
the best when compared with other thresholds. 
'''

ideal_threshold = 0.02
...
final_model = RandomForestRegressor(**hyperparameter_dict[ideal_threshold])
